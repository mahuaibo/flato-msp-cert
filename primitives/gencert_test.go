...
func TestGenCA(t *testing.T) {
	_, _, err := NewSelfSignedCert("Hyperchain", "www.hyperchain.cn", "ercert", true)
	assert.NotNil(t, err)
	// CAECC with secp256r1 private key
	ca, private, err := NewSelfSignedCert("Hyperchain", "www.hyperchain.cn", "ecert", false)
	assert.Nil(t, err)
	caca := &pem.Block{
	...
	smCa, smPrivate, err := NewSelfSignedCert("Hyperchain", "www.hyperchain.cn", "ecert", true)
	assert.Nil(t, err)
	smCaca := &pem.Block{
		Type:  pemTypeCertificate,
		Bytes: smCa,
...
-----END EC PRIVATE KEY-----`
	...
	_, err = GenCert(caStructECC, caPrivateGM.(crypto.Signer), certPrivateGM.Public(), "hyperchain", "node1", "ercert", false)
	assert.NotNil(t, err)
	_, err = GenCert(caStructECC, caPrivateGM.(crypto.Signer), certPrivateECC.Public(), "hyperchain", "node1", "rcert", false)
	assert.NotNil(t, err)
	_, err = GenCert(caStructECC, testPrivateECC.(crypto.Signer), certPrivateECC.Public(), "hyperchain", "node1", "rcert", false)
	assert.NotNil(t, err)
	//ecc ca can generate cert with ecc public key and gm public key
	_, err = GenCert(caStructECC, caPrivateECC.(crypto.Signer), certPrivateECC.Public(), "hyperchain", "node1", "rcert", false)
	assert.Nil(t, err)
	_, err = GenCert(caStructECC, caPrivateECC.(crypto.Signer), certPrivateECC.Public(), "hyperchain", "node1", "ecert", false)
	assert.Nil(t, err)
	_, err = GenCert(caStructECC, caPrivateECC.(crypto.Signer), certPrivateECC.Public(), "hyperchain", "node1", "sdkcert", false)
	assert.Nil(t, err)
	_, err = GenCert(caStructECC, caPrivateECC.(crypto.Signer), certPrivateGM.Public(), "hyperchain", "node1", "rcert", false)
	assert.Nil(t, err)
	_, err = GenCert(caStructECC, caPrivateECC.(crypto.Signer), certPrivateGM.Public(), "hyperchain", "node1", "ecert", false)
	assert.Nil(t, err)
	_, err = GenCert(caStructECC, caPrivateECC.(crypto.Signer), certPrivateGM.Public(), "hyperchain", "node1", "sdkcert", false)
	assert.Nil(t, err)
	...
	//assert.Nil(t, err)
	cert, err := GenCert(caStructGM, caPrivateGM.(crypto.Signer), certPrivateGM.Public(), "hyperchain", "node1", "rcert", false)
	assert.Nil(t, err)
...
func TestGenK1Cert(t *testing.T) {
	...
	root, err := gmx509.CreateCertificate(rand.Reader, &rootT, &rootT, rootPub, rootPriv, false)
	assert.Nil(t, err)
	...
	assert.Nil(t, err)
	ePub := ePriv.Public()
	e, err := GenCert(eT, rootPriv, ePub, "dev", "hyperchain.cn", "ecert", true)
	assert.Nil(t, err)
	...
	assert.Nil(t, err)
	rPub := rPriv.Public()
	r, err := GenCert(rT, rootPriv, rPub, "dev", "hyperchain.cn", "rcert", false)
	assert.Nil(t, err)
	...
	sdk, err := GenCert(sdkT, ePriv, sdkPub, "dev", "hyperchain.cn", "sdkcert", false)
	assert.Nil(t, err)
