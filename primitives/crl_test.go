...
func testCRLCheckRevocation(t *testing.T, url string) {
	...
	certificate, _, err := NewSelfSignedCert("Hyperchain", "www.hyperchain.cn", "ecert", false)
	assert.Nil(t, err)
...
func testCheckRevocationWithRA(t *testing.T, url string) {
	...
	assert.Equal(t, "CN=www.hyperchain.cn,OU=Hangzhou Qulian technology co., LTD,O=Hyperchain,C=ZH", getDN(cert))
