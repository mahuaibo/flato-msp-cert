...
func makeConstraintsCACert(constraints constraintsSpec, name string, key *asym.ECDSAPrivateKey, parent *Certificate, parentKey *asym.ECDSAPrivateKey) (*Certificate, error) {
	...
	derBytes, err := CreateCertificate(rand.Reader, template, parent, &key.ECDSAPublicKey, parentKey, false)
	if err != nil {
		return nil, err
	}
	...
	parse := func(constraints []string) (dnsNames []string, ips []*net.IPNet, emailAddrs []string, uriDomains []string, err error) {
		for _, constraint := range constraints {
			...
			case strings.HasPrefix(constraint, "ip:"):
				_, ipNet, error := net.ParseCIDR(constraint[3:])
				if error != nil {
...
func TestConstraintCases(t *testing.T) {
	...
	for i, test := range nameConstraintsTests {
		...
		if !test.noOpenSSL && testNameConstraintsAgainstOpenSSL && test.leaf.cn == "" {
			output, error := testChainAgainstOpenSSL(leafCert, intermediatePool, rootPool)
			if error == nil && len(test.expectedError) > 0 {
...
func testChainAgainstOpenSSL(leaf *Certificate, intermediates, roots *CertPool) (string, error) {
	...
	if debugOpenSSLFailure {
		...
		defer func() { _ = os.Remove(rootsFile.Name()) }()
	}
	...
	if len(intermediates.certs) > 0 {
		...
		if debugOpenSSLFailure {
			...
			defer func() { _ = os.Remove(intermediatesFile.Name()) }()
		}
	...
	if debugOpenSSLFailure {
		...
		defer func() { _ = os.Remove(leafFile.Name()) }()
	}
