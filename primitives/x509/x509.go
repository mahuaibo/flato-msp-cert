...
import (
	...
	"github.com/ultramesh/crypto-standard/asym/secp256k1"
...
func CreateCertificate(rand io.Reader, template, parent *Certificate, pub crypto.PublicKey, priv crypto.Signer, useGruomi bool) (cert []byte, err error) {
	...
	//如果指定生成国密证书，但是传入的公钥却不是国密的
	// todo 无法区分sm2 和ecdsa公钥
	if useGruomi && !publicKeyAlgorithm.Algorithm.Equal(oidPublicKeySM2) {
		return nil, errors.New("ra ca but non-guomi publick key")
	}
	...
	var digest []byte
	if useGruomi {
		p := priv.Public().(*gm.SM2PublicKey)
		digest = gm.HashBeforeSM2(p, tbsCertContents)
		//digest = gm.SignHashSM3(p.X.Bytes(), p.Y.Bytes(), tbsCertContents)
	} else {
		h := hashFunc.New()
