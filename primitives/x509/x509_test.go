...
import (
	...
	"github.com/ultramesh/crypto-gm"
...
func TestMarshalPublicKey(t *testing.T) {
	privateKey, err := gm.GenerateSM2Key()
	assert.Nil(t, err)
	_, _, err = marshalPublicKey(&privateKey.PublicKey)
	assert.Nil(t, err)

}

func TestString(t *testing.T) {
	assert.Equal(t, "MD2-RSA", SignatureAlgorithm(1).String())
	assert.Equal(t, "0", SignatureAlgorithm(0).String())
	assert.Equal(t, "RSA", PublicKeyAlgorithm(1).String())
	assert.Equal(t, "0", PublicKeyAlgorithm(0).String())
}

func TestRsaPSSParameters(t *testing.T) {
	value := rsaPSSParameters(Hash(5).HashFunc())
	assert.Equal(t, "3034a00f300d06096086480165030402010500a11c301a06092a864886f70d010108300d06096086480165030402010500a203020120", hex.EncodeToString(value.FullBytes))
	value = rsaPSSParameters(Hash(6).HashFunc())
	assert.Equal(t, "3034a00f300d06096086480165030402020500a11c301a06092a864886f70d010108300d06096086480165030402020500a203020130", hex.EncodeToString(value.FullBytes))
	value = rsaPSSParameters(Hash(7).HashFunc())
	assert.Equal(t, "3034a00f300d06096086480165030402030500a11c301a06092a864886f70d010108300d06096086480165030402030500a203020140", hex.EncodeToString(value.FullBytes))
}

func TestGetSignatureAlgorithmFromAI(t *testing.T) {
	ai := new(pkix.AlgorithmIdentifier)

	ai.Algorithm = asn1.ObjectIdentifier{999, 999, 999, 999, 999, 999, 999}
	ai.Parameters = rsaPSSParameters(Hash(5).HashFunc())
	assert.Equal(t, "0", getSignatureAlgorithmFromAI(*ai).String())

	ai.Algorithm = oidSignatureRSAPSS
	ai.Parameters = rsaPSSParameters(Hash(5).HashFunc())
	assert.Equal(t, "SHA256-RSAPSS", getSignatureAlgorithmFromAI(*ai).String())

	ai.Parameters = rsaPSSParameters(Hash(6).HashFunc())
	assert.Equal(t, "SHA384-RSAPSS", getSignatureAlgorithmFromAI(*ai).String())

	ai.Parameters = rsaPSSParameters(Hash(7).HashFunc())
	assert.Equal(t, "SHA512-RSAPSS", getSignatureAlgorithmFromAI(*ai).String())
}

func TestGetPublicKeyAlgorithmFromOID(t *testing.T) {
	assert.Equal(t, DSA, getPublicKeyAlgorithmFromOID(oidPublicKeyDSA))
	assert.Equal(t, ECDSA, getPublicKeyAlgorithmFromOID(oidPublicKeySM2))
	assert.Equal(t, UnknownPublicKeyAlgorithm, getPublicKeyAlgorithmFromOID(asn1.ObjectIdentifier{999, 999, 999, 999, 999, 999, 999}))
}
...
func TestError(t *testing.T) {
	e := InsecureAlgorithmError(1)
	assert.Equal(t, "x509: cannot verify signature: insecure algorithm MD2-RSA", e.Error())
...
func TestMismatchedSignatureAlgorithm(t *testing.T) {
	...
	pub := new(gm.SM2PublicKey)
	cert.PublicKey = pub
	err = cert.CheckSignature(SM3WithSM2, nil, nil)
	assert.NotNil(t, err)
}
...
func TestCreateSelfSignedCertificate(t *testing.T) {
	...
	ecdsaPriv, err := asym.GenerateKey(asym.AlgoP256R1)
	if err != nil {
	...
	tests := []struct {
		...
		{"ECDSA/RSA", &ecdsaPriv.ECDSAPublicKey, testPrivateKey, false, SHA256WithRSA},
		{"ECDSA/ECDSA", &ecdsaPriv.ECDSAPublicKey, ecdsaPriv, true, ECDSAWithSHA1},
		{"RSAPSS/RSAPSS", &testPrivateKey.PublicKey, testPrivateKey, true, SHA256WithRSAPSS},
		{"ECDSA/RSAPSS", &ecdsaPriv.ECDSAPublicKey, testPrivateKey, false, SHA256WithRSAPSS},
	...
	for _, test := range tests {
		...
		derBytes, err := CreateCertificate(random, &template, &template, test.pub, test.priv, false)
		if err != nil {
...
func TestECDSA(t *testing.T) {
	for i, test := range ecdsaTests {
		...
		if parsedKey, ok := cert.PublicKey.(*asym.ECDSAPublicKey); !ok {
...
func TestCRLCreation(t *testing.T) {
	...
	expectedCerts := []RevokedCertificate{
		{
			SerialNumber:   big.NewInt(1),
			RevocationTime: nowUTC,
		},
...
func TestCreateCertificateRequest(t *testing.T) {
	...
	ecdsa256Priv, err := asym.GenerateKey(asym.AlgoP256R1)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %s", err)
	}

	ecdsa384Priv, err := asym.GenerateKey(asym.AlgoP384R1)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %s", err)
	}

	ecdsa521Priv, err := asym.GenerateKey(asym.AlgoP521R1)
	if err != nil {
		t.Fatalf("Failed to generate ECDSA key: %s", err)
	}
...
func serialiseAndParse(t *testing.T, template *Certificate) *Certificate {
	derBytes, err := CreateCertificate(rand.Reader, template, template, &testPrivateKey.PublicKey, testPrivateKey, false)
	if err != nil {
...
func TestSystemCertPool(t *testing.T) {
	...
	a, err := x509.SystemCertPool()
	if err != nil {
		t.Fatal(err)
	}
	b, err := x509.SystemCertPool()
	if err != nil {
		t.Fatal(err)
	}
...
var testPrivateKeyECC *asym.ECDSAPrivateKey
...
func TestEmptySubject(t *testing.T) {
	...
	derBytes, err := CreateCertificate(rand.Reader, &template, &template, &testPrivateKeyECC.ECDSAPublicKey, testPrivateKeyECC, false)
	if err != nil {
		t.Fatalf("failed to create certificate: %s", err)
	}
...
func TestCheckCRLSignature(t *testing.T) {
	cert := new(Certificate)
	err := cert.CheckCRLSignature(new(CertificateList))
	assert.NotNil(t, err)
}

func TestUnhandledCriticalExtension_Error(t *testing.T) {
	e := new(UnhandledCriticalExtension)
	assert.Equal(t, "x509: unhandled critical extension", e.Error())
}

func TestGetSANExtension(t *testing.T) {
	cert := new(Certificate)

	cert.Extensions = append(cert.Extensions, pkix.Extension{ID: asn1.ObjectIdentifier{2, 5, 29, 17}, Critical: true, Value: []byte("oidExtensionSubjectAltName")})
	assert.Equal(t, []byte("oidExtensionSubjectAltName"), cert.getSANExtension())
}
...
func TestHashFunc(t *testing.T) {
	h := Hash(10)
	assert.Equal(t, crypto.Hash(10), h.HashFunc())
	assert.Equal(t, 28, h.Size())
...
func TestSigningParamsForPublicKey(t *testing.T) {
	privateKey, err := gm.GenerateSM2Key()
	assert.Nil(t, err)
	publicKey := privateKey.Public()
	h, _, err := signingParamsForPublicKey(publicKey, SM3WithSM2)
	assert.Equal(t, Hash(16), h)
	assert.Nil(t, err)
}
