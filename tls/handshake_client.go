...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		certs := make([]*gmx509.Certificate, len(certMsg.certificates))
		for i, asn1Data := range certMsg.certificates {
			cert, perr := primitives.ParseCertificate(asn1Data)
			if perr != nil {
		...
		case *rsa.PublicKey, *asym.ECDSAPublicKey, *gm.SM2PublicKey:
			break
		default:
	...
	if chainToSend != nil && len(chainToSend.Certificate) > 0 {
		...
		if signatureAlgorithm == SM2WithSM3 && hs.finishedHash.version >= VersionTLS12 {
			gmk, ok := key.(*gm.SM2PrivateKey)
			if !ok {
				_ = c.sendAlert(alertInternalError)
				return err
			}
			certVerify.signature, err = primitives.SM2Sign(gmk, hs.finishedHash.buffer)
			if err != nil {
				_ = c.sendAlert(alertInternalError)
				return err
			}
		} else {
			digest, err := hs.finishedHash.hashForClientCertificate(sigType, crypto.Hash(hashFunc), hs.masterSecret)
			if err != nil {
				_ = c.sendAlert(alertInternalError)
				return err
			}
...
findCert:
	for i, chain := range c.config.Certificates {
		...
		for j, cert := range chain.Certificate {
			...
			switch {
			case rsaAvail && x509Cert.PublicKeyAlgorithm == gmx509.RSA:
			case ecdsaAvail && x509Cert.PublicKeyAlgorithm == gmx509.ECDSA:
			default:
				continue findCert
			}
