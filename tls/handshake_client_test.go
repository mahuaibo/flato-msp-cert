...
type clientTest struct {
	...
	// *asym.ECDSAPrivateKey which is the private key for the reference server.
	key interface{}
...
func (test *clientTest) connFromCommand() (conn *recordingConn, child *exec.Cmd, stdin opensslInput, stdout *opensslOutputSink, err error) {
	...
	case *rsa.PrivateKey:
		pemType = "RSA"
		derBytes = gmx509.MarshalPKCS1PrivateKey(key)
	case *asym.ECDSAPrivateKey, *gm.SM2PrivateKey:
		pemType = "EC"
		var pmerr error
		derBytes, pmerr = primitives.MarshalPrivateKey(key)
		if pmerr != nil {
			panic(pmerr)
		}
	...
	_ = pem.Encode(&pemOut, &pem.Block{Type: pemType + " PRIVATE KEY", Bytes: derBytes})
...
func (test *clientTest) run(t *testing.T, write bool) {
	...
	if !write {
		...
		for i, b := range flows {
			...
			if !bytes.Equal(b, bb) {
				t.Fatalf("%s #%d: mismatch on read: \n got :%x\n want:%x", test.name, i, bb, b)
			}
		}
		_ = serverConn.Close()
	}
	...
	if write {
		...
		if len(recordingConn.flows) < 3 {
			_, _ = os.Stdout.Write(childProcess.Stdout.(*opensslOutputSink).all)
...
func TestHostnameInSNI(t *testing.T) {
	for _, tt := range hostnameInSNITests {
		...
		go func(host string) {
			_ = Client(c, &Config{ServerName: host, InsecureSkipVerify: true}).Handshake()
		}(tt.in)
...
func TestBuffering(t *testing.T) {
	...
	go func() {
		_ = Server(serverWCC, testConfig).Handshake()
		_ = serverWCC.Close()
		done <- true
	}()
...
func TestAlertFlushing(t *testing.T) {
	...
	go func() {
		_ = Server(serverWCC, serverConfig).Handshake()
		_ = serverWCC.Close()
		done <- true
	}()
...
func TestCloseClientConnectionOnIdleServer(t *testing.T) {
	...
	ka := rsaKeyAgreement{}
	err = ka.processServerKeyExchange(nil, nil, nil, nil, nil)
	assert.NotNil(t, err)
}
