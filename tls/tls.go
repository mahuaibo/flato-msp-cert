...
func X509KeyPair(certPEMBlock, keyPEMBlock []byte) (Certificate, error) {
	...
	case *asym.ECDSAPublicKey:
		priv, ok := cert.PrivateKey.(*asym.ECDSAPrivateKey)
		if !ok {
			return fail(errors.New("tls: private key type does not match public key type"))
		}
	...
	case *gm.SM2PublicKey:
		priv, ok := cert.PrivateKey.(*gm.SM2PrivateKey)
		if !ok {
			return fail(errors.New("tls: private key type does not match public key type"))
		}
		//if pub.X.Cmp(priv.X) != 0 || pub.Y.Cmp(priv.Y) != 0 {
		//	return fail(errors.New("tls: private key does not match public key"))
		//}
		if pub.X != priv.PublicKey.X || pub.Y != priv.PublicKey.Y {
			return fail(errors.New("tls: private key does not match public key"))
...
func parsePrivateKey(der []byte) (crypto.PrivateKey, error) {
	if key, err := primitives.UnmarshalPrivateKey(der); err == nil {
		return key, nil
	}
