...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	if len(c.peerCertificates) > 0 {
		...
		if sigAlgo == SM2WithSM3 && hs.finishedHash.version >= VersionTLS12 {
			gmpub, ok := pub.(*gm.SM2PublicKey)
			if !ok {
				_ = c.sendAlert(alertBadCertificate)
				return errors.New("tls: could not validate signature of connection nonces: validate signature failed")
			}
			pass, serr := primitives.SM2Verify(gmpub, hs.finishedHash.buffer, certVerify.signature)
			if !pass || serr != nil {
				_ = c.sendAlert(alertBadCertificate)
				return errors.New("tls: could not validate signature of connection nonces: validate signature failed")
			}
		} else {
			digest, perr = hs.finishedHash.hashForClientCertificate(sigType, crypto.Hash(hashFunc), hs.masterSecret)
			if perr == nil {
...
func (hs *serverHandshakeState) processCertsFromClient(certificates [][]byte) (crypto.PublicKey, error) {
	...
	case *asym.ECDSAPublicKey, *rsa.PublicKey, *gm.SM2PublicKey:
		pub = key
	default:
...
func (hs *serverHandshakeState) setCipherSuite(id uint16, supportedCipherSuites []uint16, version uint16) bool {
	for _, supported := range supportedCipherSuites {
		if id == supported {
			...
			if candidate.flags&suiteECDHE != 0 { //EC证书
				if !hs.ellipticOk {
					continue
				}
			...
			} else if candidate.flags&suiteSMDH != 0 && hs.sm2OK { //国密证书
				if !hs.ellipticOk {
					continue
				}
