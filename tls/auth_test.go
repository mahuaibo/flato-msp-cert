...
func TestSignatureSelection(t *testing.T) {
	...
	ecdsaCert := &testECDSAPrivateKey.ECDSAPublicKey
	...
	for testNo, test := range tests {
		...
		if test.expectedHash != crypto.Hash(hashFunc) {
	...
	sig := []byte{0}
	for testNo, test := range tests {
		_, _, hashFunc, err := pickSignatureAlgorithm(test.pubkey, test.peerSigAlgs, test.ourSigAlgs, test.tlsVersion)
		if err != nil {
			t.Errorf("test[%d]: unexpected error: %v", testNo, err)
		}
		err = verifyHandshakeSignature(signatureRSAPSS, test.pubkey, hashFunc, digest, sig)
		if err == nil {
			t.Errorf("test[%d]: unexpected error: %v", testNo, err)
		}
	}
