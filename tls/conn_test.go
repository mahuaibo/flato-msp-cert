...
func runDynamicRecordSizingTest(t *testing.T, config *Config) {
	...
	go func() {
		...
		defer func() {
			_ = clientConn.Close()
			close(recordSizesChan)
		}()

		tlsConnInner := Client(clientConn, config)
		if err := tlsConnInner.Handshake(); err != nil {
...
func TestHairpinInClose(t *testing.T) {
	...
	conn.tlsConn.OCSPResponse()
	// This call should not deadlock.
	_ = tlsConn.Close()
	extractPaddingSSL30([]byte{1})
	err := conn.tlsConn.SetDeadline(time.Now())
	assert.Nil(t, err)
