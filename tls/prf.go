...
func prf30(result, secret, label, seed []byte) {
	...
	for done < len(result) {
		...
		hashSHA1.Reset()
		_, _ = hashSHA1.Write(b[:i+1])
		_, _ = hashSHA1.Write(secret)
		_, _ = hashSHA1.Write(seed)
		digest := hashSHA1.Sum(nil)

		hashMD5.Reset()
		_, _ = hashMD5.Write(secret)
		_, _ = hashMD5.Write(digest)
...
func lookupTLSHash(signatureAlgorithm SignatureScheme) (gmx509.Hash, error) {
	switch signatureAlgorithm {
	case PKCS1WithSHA1, ECDSAWithSHA1:
		return gmx509.SHA1, nil
	case PKCS1WithSHA256, PSSWithSHA256, ECDSAWithP256AndSHA256:
		return gmx509.SHA256, nil
	case PKCS1WithSHA384, PSSWithSHA384, ECDSAWithP384AndSHA384:
		return gmx509.SHA384, nil
	case PKCS1WithSHA512, PSSWithSHA512, ECDSAWithP521AndSHA512:
		return gmx509.SHA512, nil
	case SM2WithSM3:
...
func (h *finishedHash) Write(msg []byte) (n int, err error) {
	_, _ = h.client.Write(msg)
	_, _ = h.server.Write(msg)

	if h.version < VersionTLS12 {
		_, _ = h.clientMD5.Write(msg)
		_, _ = h.serverMD5.Write(msg)
	}
...
func finishedSum30(md5, sha1 hash.Hash, masterSecret []byte, magic []byte) []byte {
	_, _ = md5.Write(magic)
	_, _ = md5.Write(masterSecret)
	_, _ = md5.Write(ssl30Pad1[:])
	md5Digest := md5.Sum(nil)

	md5.Reset()
	_, _ = md5.Write(masterSecret)
	_, _ = md5.Write(ssl30Pad2[:])
	_, _ = md5.Write(md5Digest)
	md5Digest = md5.Sum(nil)

	_, _ = sha1.Write(magic)
	_, _ = sha1.Write(masterSecret)
	_, _ = sha1.Write(ssl30Pad1[:40])
	sha1Digest := sha1.Sum(nil)

	sha1.Reset()
	_, _ = sha1.Write(masterSecret)
	_, _ = sha1.Write(ssl30Pad2[:40])
	_, _ = sha1.Write(sha1Digest)
	sha1Digest = sha1.Sum(nil)
...
func (h finishedHash) hashForClientCertificate(sigType uint8, hashAlg crypto.Hash, masterSecret []byte) ([]byte, error) {
	...
	if h.version == VersionSSL30 {
		...
		md5Hash := md5.New()
		_, _ = md5Hash.Write(h.buffer)
		sha1Hash := sha1.New()
		_, _ = sha1Hash.Write(h.buffer)
		return finishedSum30(md5Hash, sha1Hash, masterSecret, nil), nil
	}
	if h.version >= VersionTLS12 {
		hash := hashAlg.New()
		_, _ = hash.Write(h.buffer)
