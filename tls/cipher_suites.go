...
var cipherSuites = []*cipherSuite{
	...
	{GMTLS_SM2DHE_SM2SIGN_WITH_SMS4_SM3, 16, 16, 16, ecdheECDSAKA, suiteSMDH | suiteTLS12, cipherSM4, macSHA256, nil},
...
func (s ssl30MAC) MAC(digestBuf, seq, header, data, extra []byte) []byte {
	...
	s.h.Reset()
	_, _ = s.h.Write(s.key)
	_, _ = s.h.Write(ssl30Pad1[:padLength])
	_, _ = s.h.Write(seq)
	_, _ = s.h.Write(header[:1])
	_, _ = s.h.Write(header[3:5])
	_, _ = s.h.Write(data)
	digestBuf = s.h.Sum(digestBuf[:0])

	s.h.Reset()
	_, _ = s.h.Write(s.key)
	_, _ = s.h.Write(ssl30Pad2[:padLength])
	_, _ = s.h.Write(digestBuf)
...
const (
	...
	GMTLS_SM2DHE_SM2SIGN_WITH_SMS4_SM3      uint16 = 0xe011
