...
func (hc *halfConn) encrypt(b *block, explicitIVLen int) (bool, alert) {
	...
	if hc.cipher != nil {
		...
		case aead:
			...
			payloadInner := b.data[recordHeaderLen+explicitIVLen:]
			payloadInner = payloadInner[:payloadLen]
			...
			c.Seal(payloadInner[:0], nonce, payloadInner, hc.additionalData[:])
		case cbcMode:
			blockSize := c.BlockSize()
			if explicitIVLen > 0 {
				c.SetIV(payload[:explicitIVLen])
...
Again:
	...
	switch typ {
	default:
		_ = c.in.setErrorLocked(c.sendAlert(alertUnexpectedMessage))
	...
	case recordTypeAlert:
		...
		case alertLevelError:
			_ = c.in.setErrorLocked(&net.OpError{Op: "remote error", Err: alert(data[1])})
		default:
			_ = c.in.setErrorLocked(c.sendAlert(alertUnexpectedMessage))
		}
	...
	case recordTypeChangeCipherSpec:
		...
		if err != nil {
			_ = c.in.setErrorLocked(c.sendAlert(err.(alert)))
		}
...
func (c *Conn) writeRecordLocked(typ recordType, data []byte) (int, error) {
	...
	for len(data) > 0 {
		...
		_, _ = c.out.encrypt(b, explicitIVLen)
...
func (c *Conn) readHandshake() (interface{}, error) {
	...
	if n > maxHandshake {
		_ = c.sendAlertLocked(alertInternalError)
...
func (c *Conn) Write(b []byte) (int, error) {
	Infof("try to Write: conn 1029, c: %v ", string(b))
...
func (c *Conn) Read(b []byte) (n int, err error) {
	Infof("try to Read: conn 1129, isClient: %v", c.isClient)
	if err = c.Handshake(); err != nil {
		Errorf("handshake err before read: %v is client: %v ",
			c.handshakeErr.Error(), c.isClient)
		return
	}
	if len(b) == 0 {
	...
	for emptyRecordCount := 0; emptyRecordCount <= maxConsecutiveEmptyRecords; emptyRecordCount++ {
		for c.input == nil && c.in.err == nil {
			if rerr := c.readRecord(recordTypeApplicationData); rerr != nil {
			...
			if c.hand.Len() > 0 {
				...
				if herr := c.handleRenegotiation(); herr != nil {
					return 0, herr
				}
			}
		}
		if eerr := c.in.err; eerr != nil {
			return 0, eerr
		}
...
func (c *Conn) Handshake() error {
	...
	if err := c.handshakeErr; err != nil {
		Errorf("handshark err conn.go 1264: " + err.Error())
		return err
	}
	if c.handshakeComplete() {
		return nil
	...
	if c.isClient {
		...
		if c.handshakeErr != nil {
			Errorf("handshakeErr, client: " + c.handshakeErr.Error())
		}
	} else {
		Infof("server")
		c.handshakeErr = c.serverHandshake()
		if c.handshakeErr != nil {
			Errorf("handshakeErr, server: " + c.handshakeErr.Error())
		}
	}
	if c.handshakeErr == nil {
		c.handshakes++
	...
	if c.handshakeComplete() && c.handshakeErr == nil {
		Infof("handshark finish: isClient: %v", c.isClient)
	}
	return c.handshakeErr
}
