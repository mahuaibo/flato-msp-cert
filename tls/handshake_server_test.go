...
func (test *serverTest) run(t *testing.T, write bool) {
	...
	if write {
		...
		if len(recordingConn.flows) < 3 {
			_, _ = childProcess.Stdout.(*bytes.Buffer).WriteTo(os.Stdout)
			if len(test.expectHandshakeErrorIncluding) == 0 {
...
func runServerTestSSLv3(t *testing.T, template *serverTest) {
	runServerTestForVersion(t, template, "SSLv3-", "-no_ssl3")
}
...
func TestResumption(t *testing.T) {
	...
	defer func() { _ = os.Remove(sessionFilePath) }()
	...
	go func() {
		for range feeder {
			for i, f := range flows {
				...
				if err != nil {
					b.Fatalf("#%d: %s\nRead %d, wanted %d, got %x, wanted %x", i+1, err, n, len(ff), ff[:n], f)
				}
				if !bytes.Equal(f, ff) {
...
func BenchmarkHandshakeServer(b *testing.B) {
	...
	b.Run("ECDHE-P521-ECDSA-P521", func(b *testing.B) {
		if testECDSAPrivateKey.ECDSAPublicKey.Curve != elliptic.P521() {
...
func TestClientAuth(t *testing.T) {
	...
	if *update {
		...
		defer func() { _ = os.Remove(ecdsaCertPath) }()
		ecdsaKeyPath = tempFile(clientECDSAKeyPEM)
		defer func() { _ = os.Remove(ecdsaKeyPath) }()
	}
...
func TestSNIGivenOnFailure(t *testing.T) {
	...
	hello := &serverHelloMsg{nextProtoNeg: true}
	hss := serverHandshakeState{
		c:     Server(s, serverConfig),
		hello: hello,
	}
	err = hss.readFinished([]byte{1})
	assert.NotNil(t, err)
}
...
var testECDSAPrivateKey = &asym.ECDSAPrivateKey{
	ECDSAPublicKey: asym.ECDSAPublicKey{
		Curve: elliptic.P521(),
