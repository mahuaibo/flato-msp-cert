...
import (
	...
	gmpkix "github.com/ultramesh/flato-msp-cert/primitives/x509/pkix"
	"testing"
)
...
func TestPbDecrypterFor(t *testing.T) {
	...
	alg := gmpkix.AlgorithmIdentifier{
...
func TestPbDecrypt(t *testing.T) {
	for i, test := range pbDecryptTests {
		decryptable := testDecryptable{
			data: test.in,
			algorithm: gmpkix.AlgorithmIdentifier{
		...
		decryptableError := testDecryptable{
			data: test.in,
			algorithm: gmpkix.AlgorithmIdentifier{
				Algorithm: *new(asn1.ObjectIdentifier),
				Parameters: pbeParams{
					Salt:       []byte("\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8"),
					Iterations: 4096,
				}.RawASN1(),
		...
		_, err := pbDecrypt(decryptableError, nil)
		assert.NotNil(t, err)
		...
		_, err = pbDecrypt(decryptableError, password)
		assert.NotNil(t, err)

		decryptableError.data = []byte("123")
		_, err = pbDecrypt(decryptableError, password)
		assert.NotNil(t, err)
...
func TestPbEncrypt(t *testing.T) {
	...
	for i, c := range tests {
		...
		tdError := testDecryptable{
			algorithm: gmpkix.AlgorithmIdentifier{
				Algorithm: nil, // SHA1/3TDES
				Parameters: pbeParams{
					Salt:       []byte("\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8"),
					Iterations: 4096,
				}.RawASN1(),
...
func (d testDecryptable) Algorithm() gmpkix.AlgorithmIdentifier { return d.algorithm }
