...
func newSelfSignedCertWithRSA() ([]byte, interface{}, error) {
	...
	cert, err := x509.CreateCertificate(rand.Reader, &template, &template, pubKey, privKey, false)
	if err != nil {
		return nil, nil, err
	}
...
func TestPfxWithGM(t *testing.T) {
	CertGM, PrivKeyGM, err := primitives.NewSelfSignedCert("Hyperchain", "www.hyperchain.cn", "ecert", true)
	assert.Nil(t, err)
	...
	assert.Nil(t, err)
	certDer, err := x509.MarshalCertificate(cert, true)
	assert.Nil(t, err)
...
func TestPfxWithECDSA(t *testing.T) {
	CertECDSA, PrivKeyECDSA, err := primitives.NewSelfSignedCert("Hyperchain", "www.hyperchain.cn", "ecert", false)
	assert.Nil(t, err)
	...
	_, cert, _, err := DecodeChain(data, "")
	assert.Nil(t, err)
	certDer, err := x509.MarshalCertificate(cert, false)
	assert.Nil(t, err)
